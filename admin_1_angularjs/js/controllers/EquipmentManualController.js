angular.module('MetronicApp').controller('EquipmentManualController', ['$scope', '$rootScope','deviceApi','NgTableParams','$timeout','sharedataApi','$element', function($scope, $rootScope, deviceApi, NgTableParams,$timeout,sharedataApi,$element) {
    $rootScope.menueName = 'sidebar-asset';
    $scope.menueName = $rootScope.menueName;

    $scope.equipmentManualList = [];
    $scope.currentData = {};
    ...
    //操作按钮事件
    $scope.addEquipmentManual = function() {
      $('#myModal_createEquipmentManual').modal();
    };
    $scope.updateEquipmentManual = function() {
      $scope.currentData = {};
      ...
      if(checked == 0){
        $scope.message = '请选择一个设备手册';
        $('#myModal_alert').modal();
      }else if(checked > 1){
        $scope.message = '只能选择一个设备手册行编辑';
        $('#myModal_alert').modal();
      }else{
        for(var i=0; i< $scope.equipmentManualList.length; i++){
          if($scope.equipmentManualList[i].id == index){
            $scope.currentData = $scope.equipmentManualList[i];
            break;
          }
        }
        $('#myModal_updateEquipmentManual').modal();
      }
    };
    $scope.deleteEquipmentManual = function() {
      var checked = 0;
      $scope.deletelist = [];
      angular.forEach($scope.checkboxes.items, function(value,key) {
        if(value){
          ...
          let tempdata={};
          for(var i=0; i< $scope.equipmentManualList.length; i++){
            if($scope.equipmentManualList[i].id == key){
              tempdata = $scope.equipmentManualList[i];
      ...
      if(checked == 0){
        ...
        tempstr =tempstr+ '  共'+ $scope.deletelist.length+'个设备手册';
        $scope.deletestr = tempstr;
        $('#myModal_deleteEquipmentManual').modal();
      }
    };
    //modal取消事件
    $scope.createDismiss = function(){
      $('#myModal_createEquipmentManual').modal('hide');
    };
    $scope.updateDismiss = function(){
      $('#myModal_updateEquipmentManual').modal('hide');
    };
    $scope.deleteDismiss = function(){
      $('#myModal_deleteEquipmentManual').modal('hide');
    };
    ...
    //modal确定事件
    $scope.saveCreateEquipmentManual = function(){
      $('#path').val($scope.uuid);
        ...
        if(!$scope.currentData.hasOwnProperty("title") || $scope.currentData.title == ''){
            ...
            $('#myModal_createEquipmentManual').modal('hide');
            createEquipmentManualImpl();
        }
    };
    $scope.saveUpdateEquipmentManual = function(){
      if(!$scope.currentData.hasOwnProperty("title") || $scope.currentData.title == ''){
          alert('必须填写标题');
      }else if(!$scope.currentData.hasOwnProperty("description")  || $scope.currentData.description == ''){
          alert('必须填写描述');
          ...
          $('#myModal_updateEquipmentManual').modal('hide');
          updateEquipmentManualImpl();
      }
    };

    $scope.saveDeleteEquipmentManual = function(){
      $('#myModal_deleteEquipmentManual').modal('hide');
      deleteEquipmentManualImpl();
    };

    $scope.$on('$viewContentLoaded', function() {
      getEquipmentManualList();
    ...
    $scope.$watch(function() {
      ...
      angular.forEach($scope.equipmentManualList, function(item) {
    ...
    $scope.$watch(function() {
      return $scope.checkboxes.items;
        ...
        total = $scope.equipmentManualList.length;
    ...
    function getEquipmentManualList() {
        $scope.equipmentManualList = [];
        ...
        count:10
          ...
          getData: function(params) {
            return deviceApi.getEquipmentManualList('asc', (params.page()-1)*params.count(), params.count())
              .then(function(result) {
                  if(result.data.total > 0) {
                       $scope.equipmentManualList=result.data.rows;
                       for(var i=0;i<result.data.rows.length;i++) {
                         $scope.equipmentManualList[i].pathURI = uploaderUrl+'/files/'+$scope.equipmentManualList[i].path;
                       }
                  }else {
                    $scope.equipmentManualList=[];
                  }
                  params.total(result.data.total);
                  return $scope.equipmentManualList;
              }, function(err) {
                console.log('获取设备手册列表err',err);
              });
          }
        });
    ...
    function createEquipmentManualImpl() {
      var params={};
      ...
      deviceApi.createEquipmentManual(params)
          .then(function(result){
              if(result.data.code == 1 ){
                $scope.message = '新建设备手册成功！';
                $('#myModal_alert').modal();
                getEquipmentManualList();
              }else{
              ...
              console.log('设备手册列表获取err', err);
          });
    }

    function updateEquipmentManualImpl(){
      var params={};
      params.title = $scope.currentData.title;
      params.description = $scope.currentData.description;
      params.content = $scope.currentData.content;
      ...
      deviceApi.updateEquipmentManual($scope.currentData.id,params)
          .then(function(result){
              if(result.data.code == 1 ){
                $scope.message = '设备手册修改成功！';
                $('#myModal_alert').modal();
                getEquipmentManualList();
              }else{
                $scope.message = result.data.message;
              ...
              console.log('设备手册修改err', err);
          });
    }

    function deleteEquipmentManualImpl(){
      var ids='';
      for(var i=0; i< $scope.deletelist.length; i++){
        ids =ids+ $scope.deletelist[i].id+'::';
      }
      deviceApi.deleteEquipmentManual(ids)
      .then(function(result){
          if(result.data.code ==1 ){
            $scope.message = '设备手册删除成功！';
            $('#myModal_alert').modal();
            getEquipmentManualList();
          }else{
            $scope.message = result.data.message;
